...
export function JobDetailsPage({ slug }: { slug: string }) {
  ...
  const applicationForm = useForm({
    defaultValues: {
      ...
      resumeText: user?.userProfile?.resumeText ?? "",
      coverLetter: "",
    },
  });
  ...
  const handleGenerateCoverLetter = async () => {
    ...
    try {
      const { data } = await api.post("/ai/generate-cover-letter", { jobId: job._id});
      
      const result = data.data;
      console.log(result);
      applicationForm.setValue('coverLetter', result.coverLetter)
        ...
        <form
          onSubmit={applicationForm.handleSubmit(async (values) => {
            ...
            await applyMutation.mutateAsync({
              ...
              resumeText: user?.userProfile?.resumeText || undefined,
