...
usersRoutes.post("/me/saved-jobs/:jobId", protect, restrictTo("user"), validateRequest(savedJobParamSchema), usersController.saveJob,);
usersRoutes.delete("/me/saved-jobs/:jobId", protect, restrictTo("user"), validateRequest(savedJobParamSchema), usersController.unsaveJob,);

usersRoutes.get("/dashboard", protect, usersController.getUserDashboard);
usersRoutes.get("/recruiter/overview", protect, restrictTo("recruiter"), usersController.getRecruiterDashboard);
